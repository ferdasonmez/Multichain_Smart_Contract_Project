...
try {
  const cliClientCommand = `cross-env TITLE="${network} Community CLI Client" npx hardhat run --network ${network} ./scripts/application_frontend.ts`;
...
try {
  const eventsListenerCommand = `cross-env TITLE="${network} Community Events Listener" npx hardhat run --network ${network} ./scripts/get_community_events.ts`;
